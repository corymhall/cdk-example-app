 ...
 */
export interface AddFunctionRouteOptions
  extends Omit<
    AddRoutesOptions,
 ...
 */
export interface AddServiceRouteOptions
  extends Omit<
    AddRoutesOptions,
    'integration' | 'authorizer' | 'authorizationScopes'
  > {
...
export class Api extends HttpApi implements IConnectable {
   ...
   */
  public addServiceRoute(
    id: string,
    options: AddServiceRouteOptions,
  ): HttpRoute[] {
    const route = super.addRoutes({
      ...options,
      integration: new HttpServiceDiscoveryIntegration(
        id,
        options.app.service.cloudMapService!,
        {
  ...
  public addLambdaRoute(
    id: string,
    options: AddFunctionRouteOptions,
  ): HttpRoute[] {
