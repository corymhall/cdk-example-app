...
export class DynamoDBMonitor implements IMonitorComponent {
  ...
  constructor(
    ...
    this.userErrorsMetric = this.table.metricUserErrors({ label: 'user errors' });
    this.successfulRequestLatencyMetric = this.table.metricSuccessfulRequestLatency({ label: 'successful request latency', dimensionsMap: { Operation: Operation.GET_ITEM } });
    this.conditionChecksFailedMetric = this.table.metricConditionalCheckFailedRequests({ label: 'condition checks failed' });
    this.getItemErrorMetrics = this.table.metricSystemErrorsForOperations({ operations: [Operation.GET_ITEM], label: 'get item error' });
    this.putItemErrorMetrics = this.table.metricSystemErrorsForOperations({ operations: [Operation.PUT_ITEM], label: 'put item error' });
    this.getItemThrottledMetric = this.table.metricThrottledRequestsForOperations({ operations: [Operation.GET_ITEM], label: 'get item throttled' });
    this.putItemThrottledMetric = this.table.metricThrottledRequestsForOperations({ operations: [Operation.PUT_ITEM], label: 'put item throttled' });
    this.readCapacityMetric = this.table.metricConsumedReadCapacityUnits({ label: 'read capacity' });
    this.writeCapacityMetric = this.table.metricConsumedWriteCapacityUnits({ label: 'write capacity' });
  };
  bind(facade: MonitoringFacade): IDynamicDashboardSegment {
    ...
    const successfulLatencyMetric = metricFactory
    ...
    anomalyFactory.addAlarmWhenOutOfBand(successfulLatencyMetric, 'SuccessLatency-Anomaly', 'Critical',
