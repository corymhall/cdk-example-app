...
export class ApiMonitor implements IMonitorComponent {
  ...
  constructor(
    ...
    this.integrationLatencyMetric = this.api.metricIntegrationLatency({
    ...
    this.clientErrorMetric = this.api.metricClientError({
      label: 'client error',
    });
    this.serverErrorMetric = this.api.metricServerError({
      label: 'server error',
    });
  }
  bind(facade: MonitoringFacade): IDynamicDashboardSegment {
    ...
    const integrationLatencyMetric = metricFactory.createMetricAnomalyDetection(
